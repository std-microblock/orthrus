...
namespace HitmanPatcher
{
	public static class v1_15
	{
		...
		private static HitmanVersion v1_15_0_dx11 = new HitmanVersion()
		{
			certpin = new[] { new Patch(0x0CD744C, "0F85", "90E9", MemProtection.PAGE_EXECUTE_READ) },
			authheader = new[]
			{
			...
			configdomain = new[] { new Patch(0x273C628, "", "", MemProtection.PAGE_READWRITE, "configdomain") },
			protocol = new[]
			{
				new Patch(0x14DD458, "68", "61", MemProtection.PAGE_READONLY) // this is just stupid
			},
			dynres_noforceoffline = new[] { new Patch(0x273CAA8, "01", "00", MemProtection.PAGE_READWRITE) }
		};

		private static HitmanVersion v1_15_0_dx12 = new HitmanVersion()
		{
			certpin = new[] { new Patch(0x0CD7C6C, "0F85", "90E9", MemProtection.PAGE_EXECUTE_READ) },
			authheader = new[]
			{
			...
			configdomain = new[] { new Patch(0x2744B28, "", "", MemProtection.PAGE_READWRITE, "configdomain") },
			protocol = new[]
			{
				new Patch(0x14E44B8, "68", "61", MemProtection.PAGE_READONLY) // this is just stupid
			},
			dynres_noforceoffline = new[] { new Patch(0x2744FA8, "01", "00", MemProtection.PAGE_READWRITE) }
		};
	}
}
